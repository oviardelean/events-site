...
class ValidatorTest extends TestBase {
  ...
  public function testValidate() {
    ...
    $validator = new Validator(\Drupal::getContainer()->getParameter('app.root'), \Drupal::service('module_handler'), \Drupal::service('theme_handler'), \Drupal::service('transliteration'), 'minimal', \Drupal::service('extension.list.module'));
  ...
  public function testValidateExtensionsInProfile() {
    ...
    $validator = new Validator(\Drupal::getContainer()->getParameter('app.root'), \Drupal::service('module_handler'), \Drupal::service('theme_handler'), \Drupal::service('transliteration'), 'testing', \Drupal::service('extension.list.module'));
    $validator->validate('test', 'test');
  }
  ...
  public function testValidateThemeNameClash() {
    $this->expectExceptionMessage('The machine name stark already exists');
    $this->installProfile('minimal');
    $validator = new Validator(\Drupal::getContainer()->getParameter('app.root'), \Drupal::service('module_handler'), \Drupal::service('theme_handler'), \Drupal::service('transliteration'), 'minimal', \Drupal::service('extension.list.module'));
    $validator->validate('test', 'stark');
  }

  /**
   * Installs an install profile for testing.
   *
   * @param string $name
